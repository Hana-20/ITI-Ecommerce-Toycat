...
public class ProductRepository {
    ...
    public List<Product> searchByCategory(String categoryString) {
        ...
        //     case "Action_Figures":
        //     products = entityManager.createQuery("SELECT p FROM Product p, Categroy c WHERE p.category_id=1", Product.class).getResultList();
        //     break;
        //     case "Dolls_and_Accessories":
        //     products = entityManager.createQuery("SELECT p FROM Product p, Categroy c WHERE p.category_id=2", Product.class).getResultList();
        //     break;
        //     case "Building_Toys":
        //     products = entityManager.createQuery("SELECT p FROM Product p, Categroy c WHERE p.category_id=3", Product.class).getResultList();
        //     break;
      ...
      //  products = entityManager.createQuery("SELECT p FROM Product p, Categroy c WHERE p.category_id=6", Product.class).getResultList();

        products = entityManager.createQuery("SELECT p FROM Product p WHERE p.categoryId.name = :specificCategoryName", Product.class).setParameter("specificCategoryName",categoryString).getResultList();
